...
import { useGetDetilMutation } from "../../../../../redux/services/admin/users-management/rolesPage";
...
export default function ModalDetilRole(props: detilProps) {
   ...
   useEffect(
      function () {
         if (props.show && props.role_id) {
            (async function () {
               setLoading(true);
               try {
                  let rsp: any = await getDetail({
                     detil: parseInt((props.role_id || 0).toString()),
                  });
                  let { data = {} }: any = rsp
                  if (data.code !== "00")
                     disp(
                        layoutActions.openAlert({
                           title: `Error get detail Role`,
                           type: "Warning",
   ...
   return (
      ...
      >
         <Modal.Header closeBtn>Detail Roles</Modal.Header>
         <Modal.Body>
            <TableDetail
               item={detil}
               fieldList={[
